...
function parseRemoteUrl(url: string): { provider: GitContext['provider']; owner: string; name: string } {
  let provider: GitContext['provider'] = 'unknown';
  let owner = '';
  let name = '';
  ...
  if (match) {
    ...
    else if (host.includes('azure') || host.includes('visualstudio')) provider = 'azure';
...
export function validateGitContext(context: GitContext): void {
  if (!context.repoRoot) {
