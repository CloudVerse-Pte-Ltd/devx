...
exports.renderQuiet = renderQuiet;
const COLORS = {
    reset: '\x1b[0m',
    bold: '\x1b[1m',
    dim: '\x1b[2m',
...
function formatRebuttalStatus(finding) {
    if (!finding.rebuttalStatus)
        return '';
...
function formatCostSignalSummary(response) {
    const lines = [colorize('Summary', COLORS.bold)];
...
function formatNextActions(response) {
    if (response.findings.length === 0)
        return [];
    const lines = [colorize('Next steps', COLORS.bold)];
...
function renderTable(response) {
    ...
    if (response.usage) {
        lines.push(colorize(`Usage: ${response.usage.unitsConsumed} units consumed, ${response.usage.unitsRemaining} remaining`, COLORS.dim));
        lines.push('');
    }
    return lines.join('\n');
}
